...
const handleSubmit = async (e) => {
    ...
    try {
        ...
        const studentResponse = await axios.post(`http://localhost:8081/api/enroll/${userId}/${academyId}/${courseId}`, formData);
        if (studentResponse.status === 200) {
            // Student enrolled successfully, now create enrollment record
            const enrollmentData = { userId, ...formData }; // Include userId in enrollment data
            const enrollmentResponse = await axios.post('http://localhost:8081/api/enrollments', enrollmentData);
